...
def reparar_mojibake_total(df):
    """
    Repara TODO el contenido de un DataFrame aplicando:
    texto_roto.encode('latin1').decode('utf-8')
    Esto arregla 'ResoluciÃ³n', 'PequeÃ±a', 'nÂº', etc.
    """
    def fix(x):
...
def leer_csv_utf8_fuerte(path):
    """
    Lee un CSV estilo Power BI:
    - prueba utf-8
    - prueba utf-8-sig
    - aplica reparación general
    """
    try:
...
# Reparación final (por si algo quedó colgado)
df_digesto_normas = reparar_mojibake_total(df_digesto_normas)
...
# Mojibake final por las dudas
df_digesto_rel = reparar_mojibake_total(df_digesto_rel)
